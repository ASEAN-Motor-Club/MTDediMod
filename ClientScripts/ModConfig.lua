...
local function LoadModConfig(force)
    ...
    if file then
        ...
        if success and type(content) == "table" then
            ...
            -- This handles nested tables (e.g. shortcuts) properly
            ...
            -- Only merge keys that exist in our defaults
            for key, _ in pairs(modConfig) do
                if content[key] ~= nil then
                    if type(modConfig[key]) == "table" and type(content[key]) == "table" then
                        deepMerge(modConfig[key], content[key])
                    else
